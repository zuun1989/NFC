            ...
            if (length == 60 || (length == 59 && str.charAt(2) == '$')) {
                ...
                if (allowedVersions.contains(str2)) {
                    ...
                    try {
                        ...
                        String doGenerate = doGenerate(str2, bArr, decodeSaltString(str.substring(str.lastIndexOf(36) + 1, length - 31)), parseInt);
                        if (length == doGenerate.length()) {
                            z = true;
                        } else {
                            z = false;
                        }
                        for (int i2 = 0; i2 != length; i2++) {
                            if (str.charAt(i2) == doGenerate.charAt(i2)) {
                                z2 = true;
                            } else {
                                z2 = false;
                            }
    ...
    private static void encodeData(StringBuilder sb2, byte[] bArr) {
        ...
        if (bArr.length == 24 || bArr.length == 16) {
            ...
            for (int i = 0; i < length; i += 3) {
                ...
                sb2.append((char) bArr3[(b >>> 2) & 63]);
                sb2.append((char) bArr3[((b << 4) | (b2 >>> 4)) & 63]);
                sb2.append((char) bArr3[((b2 << 2) | ((b3 & 255) >>> 6)) & 63]);
                sb2.append((char) bArr3[b3 & 63]);
            }
            if (z) {
