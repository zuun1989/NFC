...
public class GCMSIVBlockCipher implements AEADBlockCipher {
    ...
    private static void checkBuffer(byte[] bArr, int i, int i2, boolean z) {
        ...
        if (i2 < 0 || i < 0 || i3 < 0 || i3 > bufLength) {
            if (z) {
                th = new OutputLengthException("Output buffer too short.");
            } else {
                th = new DataLengthException("Input buffer too short.");
            }
            throw th;
        }
    }
    ...
    private void resetStreams() {
        ...
        GCMSIVCache gCMSIVCache2 = this.thePlain;
        if (gCMSIVCache2 != null) {
            gCMSIVCache2.clearBuffer();
        }
