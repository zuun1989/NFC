...
public final class XMSSMTSignature implements XMSSStoreableObjectInterface, Encodable {
    ...
    public byte[] toByteArray() {
        ...
        for (XMSSReducedSignature byteArray : this.reducedSignatures) {
            XMSSUtil.copyBytesAtOffset(bArr, byteArray.toByteArray(), i);
            i += height;
        }
        return bArr;
    }
            ...
            if (access$100 != null) {
                ...
                if (access$100.length == ceil + treeDigestSize + (access$000.getLayers() * height)) {
                    ...
                    if (XMSSUtil.isIndexValid(access$000.getHeight(), bytesToXBigEndian)) {
                        this.random = XMSSUtil.extractBytesAtOffset(access$100, ceil, treeDigestSize);
                        this.reducedSignatures = new ArrayList();
                        for (int i = ceil + treeDigestSize; i < access$100.length; i += height) {
                            this.reducedSignatures.add(new XMSSReducedSignature.Builder(this.params.getXMSSParameters()).withReducedSignature(XMSSUtil.extractBytesAtOffset(access$100, i, height)).build());
                        }
                        return;
                    }
