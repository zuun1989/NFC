...
public class KeyFactorySpi extends BaseKeyFactorySpi {
    ...
    public KeySpec engineGetKeySpec(Key key, Class cls) throws InvalidKeySpecException {
        ...
        } else if (cls.isAssignableFrom(com.fis.ekyc.nfc.build_in.bouncycastle.jce.spec.OpenSSHPublicKeySpec.class) && (key instanceof DSAPublicKey)) {
            DSAPublicKey dSAPublicKey3 = (DSAPublicKey) key;
            try {
                return new com.fis.ekyc.nfc.build_in.bouncycastle.jce.spec.OpenSSHPublicKeySpec(OpenSSHPublicKeyUtil.encodePublicKey(new DSAPublicKeyParameters(dSAPublicKey3.getY(), new DSAParameters(dSAPublicKey3.getParams().getP(), dSAPublicKey3.getParams().getQ(), dSAPublicKey3.getParams().getG()))));
            } catch (IOException e3) {
                throw new IllegalArgumentException("unable to produce encoding: " + e3.getMessage());
            }
        } else if (!cls.isAssignableFrom(com.fis.ekyc.nfc.build_in.bouncycastle.jce.spec.OpenSSHPrivateKeySpec.class) || !(key instanceof DSAPrivateKey)) {
            return super.engineGetKeySpec(key, cls);
        } else {
            DSAPrivateKey dSAPrivateKey3 = (DSAPrivateKey) key;
            try {
                return new com.fis.ekyc.nfc.build_in.bouncycastle.jce.spec.OpenSSHPrivateKeySpec(OpenSSHPrivateKeyUtil.encodePrivateKey(new DSAPrivateKeyParameters(dSAPrivateKey3.getX(), new DSAParameters(dSAPrivateKey3.getParams().getP(), dSAPrivateKey3.getParams().getQ(), dSAPrivateKey3.getParams().getG()))));
