...
public enum zzald {
    ...
    MAP(50, zzalf.MAP, zzalo.VOID);
    
    private static final zzald[] zzaz = null;
    private final int zzbb;

    static {
        zzald[] values = values();
        zzaz = new zzald[values.length];
        for (zzald zzald : values) {
            zzaz[zzald.zzbb] = zzald;
        }
    }

    private zzald(int i, zzalf zzalf, zzalo zzalo) {
        this.zzbb = i;
        int ordinal = zzalf.ordinal();
        if (ordinal == 1) {
            zzalo.zza();
        } else if (ordinal == 3) {
            zzalo.zza();
        }
        if (zzalf == zzalf.SCALAR) {
            int i2 = zzalc.zza[zzalo.ordinal()];
        }
    }
